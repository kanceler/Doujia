...
func TestMemoryRunRepositoryListOrdersNewestFirst(t *testing.T) {
	ctx := context.Background()
	repository := NewMemoryRunRepository()
	older := time.Date(2026, 5, 1, 9, 0, 0, 0, time.UTC)
	newer := older.Add(time.Hour)
	for _, run := range []RunRecord{
		{ID: "run_old", PipelineID: "phase_two_delivery_flow", Status: core.RunStatusCreated, ProjectDir: t.TempDir(), CreatedAt: older, UpdatedAt: older},
		{ID: "run_new", PipelineID: "phase_two_delivery_flow", Status: core.RunStatusRunning, ProjectDir: t.TempDir(), CreatedAt: newer, UpdatedAt: newer},
	} {
		if err := repository.Create(ctx, run); err != nil {
			t.Fatalf("Create(%s) error = %v", run.ID, err)
		}
	}
	...
	if len(runs) != 2 || runs[0].ID != "run_new" || runs[1].ID != "run_old" {
