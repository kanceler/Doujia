...
func (dockerContainerRunner) Create(ctx context.Context, name string, image string, workspaceDir string) (string, error) {
	runCtx, cancel := createContainerCommandContext(ctx)
	defer cancel()
	if _, err := runHostCommand(runCtx, "docker", "rm", "-f", name); err != nil && !isDockerNoSuchContainerError(err) {
		return "", err
	}
...
func isDockerNoSuchContainerError(err error) bool {
	if err == nil {
		return false
	}
	text := strings.ToLower(err.Error())
	return strings.Contains(text, "no such container") || strings.Contains(text, "cannot remove container")
}
