...
func TestContainerGitWorktreePrepareInitializesOwnedPathsInWorktree(t *testing.T) {
	t.Parallel()
	...
	writeJSONFile(t, modulePath, map[string]any{
		"module_id":           "module01",
		"module_name":         "snake-game-spa",
		"module_role":         "frontend",
		"implementation_role": "front",
		"branch_name":         "feature/module01-frontend",
		"worktree_dir":        "/workspace/worktrees/module01",
		"owned_paths":         []string{"index.html", "package.json", "src/**"},
		"runtime_write_paths": []string{"src/tests/**"},
		"forbidden_paths":     []string{"server/**", "README.md", ".git/**"},
		"test_command":        "npm test",
		"complexity":          "high",
	})
	...
	runner := scriptedContainerRunner{
		execFunc: func(_ context.Context, _ string, args []string, _ string) (createContainerCommandResult, error) {
			...
			for _, want := range []string{
				"git -C \"$repo_dir\" worktree add -- \"$worktree_dir\" \"$branch_name\"",
				"prepare_module_path \"$worktree_dir\" 'index.html' 'file'",
				"prepare_module_path \"$worktree_dir\" 'package.json' 'file'",
				"prepare_module_path \"$worktree_dir\" 'src' 'dir'",
				"prepare_module_path \"$worktree_dir\" 'src/tests' 'dir'",
			} {
				if !strings.Contains(command, want) {
					t.Fatalf("command = %q, want substring %q", command, want)
				}
...
func TestContainerGitWorktreePrepareSkipsRuntimePathNestedUnderOwnedFile(t *testing.T) {
	t.Parallel()
	...
	writeJSONFile(t, modulePath, map[string]any{
		...
		"branch_name":         "feature/module01-frontend",
		"worktree_dir":        "/workspace/worktrees/module01",
		"owned_paths":         []string{"index.html", "src/**", "public/**", "package.json"},
		"runtime_write_paths": []string{"index.html/tests/**"},
		"test_command":        "npm test",
		"complexity":          "high",
	})
	...
	runner := scriptedContainerRunner{
		execFunc: func(_ context.Context, _ string, args []string, _ string) (createContainerCommandResult, error) {
			...
			for _, want := range []string{
				"prepare_module_path \"$worktree_dir\" 'index.html' 'file'",
				"prepare_module_path \"$worktree_dir\" 'src' 'dir'",
				"prepare_module_path \"$worktree_dir\" 'public' 'dir'",
				"prepare_module_path \"$worktree_dir\" 'package.json' 'file'",
			} {
				if !strings.Contains(command, want) {
					t.Fatalf("command = %q, want substring %q", command, want)
			...
			if strings.Contains(command, "prepare_module_path \"$worktree_dir\" 'index.html/tests' 'dir'") {
				t.Fatalf("command must not create runtime path under owned file index.html: %q", command)
			}
			return createContainerCommandResult{Stdout: "created\n"}, nil
		},
	}
