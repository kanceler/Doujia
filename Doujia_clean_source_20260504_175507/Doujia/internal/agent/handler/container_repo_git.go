...
func handleContainerGitWorktreePrepare(ctx context.Context, req core.HandlerRequest, runner containerRunner) (core.HandlerResponse, error) {
	...
	ownedPathLines, err := moduleOwnedPathScriptLines(resolved.spec)
	if err != nil {
		return core.HandlerResponse{}, err
	}
	...
	scriptLines = append(scriptLines, indentShellLines(ownedPathLines, "  ")...)
	scriptLines = append(scriptLines,
	...
	scriptLines = append(scriptLines, ownedPathLines...)
	scriptLines = append(scriptLines,
		"printf 'created\\n'",
	)
...
func moduleOwnedPathScriptLines(spec containerModuleSpec) ([]string, error) {
	lines := []string{
		"prepare_module_path() {",
		"  root=\"$1\"",
		"  rel=\"$2\"",
	...
	ownedFilePaths := map[string]bool{}
	for _, rawPath := range spec.OwnedPaths {
		relativePath, kind, ok, err := modulePathBootstrapTarget(rawPath)
		if err != nil {
			return nil, err
		}
		if ok && kind == "file" {
			ownedFilePaths[relativePath] = true
		}
	}
	added := map[string]bool{}
	for _, rawPath := range append(append([]string{}, spec.OwnedPaths...), spec.RuntimeWritePaths...) {
		relativePath, kind, ok, err := modulePathBootstrapTarget(rawPath)
		if err != nil {
			return nil, err
		}
		if !ok || added[kind+":"+relativePath] {
			continue
		}
		if kind == "dir" && pathNestedUnderFile(relativePath, ownedFilePaths) {
			continue
		}
		added[kind+":"+relativePath] = true
		lines = append(lines, "prepare_module_path \"$worktree_dir\" "+shellQuote(relativePath)+" "+shellQuote(kind))
	}
	return lines, nil
}

func pathNestedUnderFile(relativePath string, filePaths map[string]bool) bool {
	for filePath := range filePaths {
		if relativePath == filePath || strings.HasPrefix(relativePath, filePath+"/") {
			return true
		}
	}
	return false
...
func modulePathBootstrapTarget(rawPath string) (string, string, bool, error) {
	...
	relativePath, err := normalizeContainerRelativePath(rawPath)
	if err != nil {
		return "", "", false, fmt.Errorf("module owned path bootstrap target %q: %w", rawPath, err)
	}
	if relativePath == ".git" || strings.HasPrefix(relativePath, ".git/") {
		return "", "", false, fmt.Errorf("module owned path bootstrap target %q escapes into .git", rawPath)
	}
	return relativePath, kind, true, nil
}

func indentShellLines(lines []string, prefix string) []string {
