...
type PluginRegistry struct {
	...
	handlerRegs map[string]core.HandlerRegistration
	roleSpecs map[string]core.RoleSpec
	roleRegs  map[string]core.RoleRegistration
...
func (r *PluginRegistry) RegisterRole(reg core.RoleRegistration) error {
	...
	r.roleRegs[reg.Spec.ID] = reg
	return nil
}

func (r *PluginRegistry) HandlerRegistrations() []core.HandlerRegistration {
	if len(r.handlerRegs) == 0 {
		return nil
	}
	ids := make([]string, 0, len(r.handlerRegs))
	for id := range r.handlerRegs {
		ids = append(ids, id)
	}
	sort.Strings(ids)
	out := make([]core.HandlerRegistration, 0, len(ids))
	for _, id := range ids {
		out = append(out, r.handlerRegs[id])
	}
	return out
}

func (r *PluginRegistry) HandlerSpecs() []core.HandlerSpec {
	regs := r.HandlerRegistrations()
	if len(regs) == 0 {
		return nil
	}
	out := make([]core.HandlerSpec, 0, len(regs))
...
func (r *PluginRegistry) OpRegistrations() []core.OpRegistration {
	if len(r.opRegs) == 0 {
		return nil
	}
	ids := make([]string, 0, len(r.opRegs))
	for id := range r.opRegs {
		ids = append(ids, id)
	}
	sort.Strings(ids)
	out := make([]core.OpRegistration, 0, len(ids))
	for _, id := range ids {
		out = append(out, r.opRegs[id])
	}
	return out
}

func (r *PluginRegistry) RoleRegistrations() []core.RoleRegistration {
	if len(r.roleRegs) == 0 {
		return nil
	}
	ids := make([]string, 0, len(r.roleRegs))
	for id := range r.roleRegs {
		ids = append(ids, id)
	}
	sort.Strings(ids)
	out := make([]core.RoleRegistration, 0, len(ids))
	for _, id := range ids {
		out = append(out, r.roleRegs[id])
	}
	return out
}
