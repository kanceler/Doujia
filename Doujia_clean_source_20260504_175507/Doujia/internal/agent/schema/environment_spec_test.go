...
func TestEnvironmentSpecValidateRejectsCheckCommandUnavailableInImage(t *testing.T) {
	t.Parallel()
	...
	spec := EnvironmentSpec{
		...
		Image:              "node:20-bookworm",
		PackageManager:     "npm",
		CheckCommands:      []string{"node --version", "go version"},
		DefaultTestCommand: "npm test",
	}

	err := spec.Validate()
	if err == nil {
		t.Fatal("Validate() error = nil, want image/check command mismatch")
	}
	if !strings.Contains(err.Error(), "check_command_not_available_in_image") {
		t.Fatalf("Validate() error = %v, want check_command_not_available_in_image", err)
	}
}
