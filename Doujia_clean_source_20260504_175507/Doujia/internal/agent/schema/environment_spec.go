...
func (s EnvironmentSpec) Validate() error {
	...
	for _, command := range s.CheckCommands {
		if !CheckCommandAvailableInImage(command, image) {
			return fmt.Errorf("check_command_not_available_in_image: %s not available in %s", command, image)
		}
	}
...
func CheckCommandAvailableInImage(command, image string) bool {
	...
	switch image {
	case "node:20-bookworm":
		return command == "node --version" || command == "npm --version"
	case "python:3.12-bookworm":
		return command == "python --version" || command == "python3 --version" || command == "pip --version"
	case "golang:1.23-bookworm", "golang:1.22-bookworm":
...
- 必填字段：
  ...
  - "check_commands": 字符串数组，至少 1 项；每一项必须是允许命令之一：node --version、npm --version、git --version、python --version、python3 --version、pip --version、go version、curl --version；命令必须与 image 匹配，node:20-bookworm 不允许 go version，golang 镜像才允许 go version，python 镜像才允许 python/pip 检查命令
