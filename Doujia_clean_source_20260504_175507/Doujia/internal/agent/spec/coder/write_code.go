...
func WriteCodeSpec() core.OpSpec {
	return core.OpSpec{
		...
		OpDescription:   "Read container_context, module_spec, coder_task, module_contract, and seed_tests. Before the LLM tool loop begins, the runtime must run a fixed preflight step that prepares the real Git branch and worktree using container_context.repo_dir, container_context.base_branch, module_spec.branch_name, and module_spec.worktree_dir. After that preflight succeeds, the agent must read upstream artifacts with artifact_read, inspect existing files with container_read, modify code and tests under module_spec.worktree_dir with container_write, execute module_spec.test_command or other necessary checks with container_run, commit the module changes with container_git_commit, write coder_branch.json with artifact_write, and finish through task_complete. Missing files or directories under owned_paths are normal for a new local project: create them with container_write and continue. Do not fail solely because expected owned_paths do not already exist or because unrelated existing scaffold files are outside owned_paths; leave unrelated and forbidden paths untouched. The implementation strategy must follow module_spec.complexity: low complexity should minimize extra exploration after the required reads, while high complexity should spend more effort understanding existing code and boundaries before implementation. Generated and modified files must stay within module_spec.owned_paths or module_spec.runtime_write_paths, and must avoid module_spec.forbidden_paths. coder_branch must record real execution facts including module_id, container_id, repo_dir, base_branch, base_commit, branch, commit, worktree, changed_files, test_command, result, and test_passed.",
...
func DebugWriteCodeSpec() core.OpSpec {
	...
	spec.OpDescription = "Repair a module coder branch after tester.test_code returned kbug. Read container_context, module_spec, coder_task, module_contract, seed_tests, the previous coder_branch when present, and the upstream_artifact_issue or module_test_report that explains the failure. Prepare the same real Git worktree, make the smallest necessary repair, rerun the relevant checks, commit the fixed module changes, and write a fresh coder_branch.json. The new coder_branch must include module_id, container_id, repo_dir, base_branch, base_commit, branch, commit, worktree, changed_files, test_command, result, and test_passed."
