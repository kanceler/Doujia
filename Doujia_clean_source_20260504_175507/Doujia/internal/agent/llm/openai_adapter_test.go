...
func TestOpenAIAdapterStreamChatEmitsDeltasAndRequestsStream(t *testing.T) {
	t.Parallel()
	...
	var deltas []string
	resp, err := adapter.StreamChat(context.Background(), ChatRequest{
		Messages: []Message{{Role: "user", Content: "ping"}},
	...
	if resp.Message.Content != "hello world" {
		t.Fatalf("StreamChat() content = %q, want hello world", resp.Message.Content)
	}
}
...
func TestOpenAIAdapterStreamChatUsesResponsesStreamingEvents(t *testing.T) {
	t.Parallel()
...
func TestOpenAIAdapterApplyConfigOverridesEnvFields(t *testing.T) {
	...
	adapter := NewOpenAIAdapterFromEnv()
	adapter.ApplyConfig(OpenAIAdapterConfig{
		BaseURL:  "https://run.example.test/v1",
		APIKey:   "run-key",
		Model:    "run-model",
		APIStyle: "chat_completions",
	})

	if adapter.BaseURL != "https://run.example.test/v1" {
		t.Fatalf("BaseURL = %q, want run value", adapter.BaseURL)
	}
	if adapter.APIKey != "run-key" {
		t.Fatalf("APIKey = %q, want run value", adapter.APIKey)
	}
	if adapter.Model != "run-model" {
		t.Fatalf("Model = %q, want run value", adapter.Model)
	}
	if adapter.APIStyle != "chat_completions" {
		t.Fatalf("APIStyle = %q, want run value", adapter.APIStyle)
	}
}
