...
func (a *Agent) Run(ctx context.Context, req core.AgentRunRequest) (core.AgentResult, error) {
	...
	default:
		return agentFail("unsupported_architect_op", "architect does not support op: "+req.Task.Op), nil
	}
}
...
func (a *Agent) runMergeCode(ctx context.Context, req core.AgentRunRequest) (core.AgentResult, error) {
	...
	for _, module := range moduleSpecs.Modules {
		...
		branch, err := readMergeInput[mergeCoderBranch](req.Bundle, core.ModuleCoderBranchKey(moduleID))
		if err != nil {
		...
		report, err := readMergeInput[mergeModuleReport](req.Bundle, core.ModuleTestReportKey(moduleID))
		if err != nil {
...
func validateMergePreconditions(container mergeContainerContext, branches []mergeCoderBranch, reports []mergeModuleReport) ([]string, map[string]any) {
	...
	for i, report := range reports {
		...
		if strings.TrimSpace(branch.Result) != "" && branch.Result != "kok" {
	...
	if len(baseCommitToModules) > 1 {
		issues = append(issues, "coder branches were created from different base_commit values")
	}
