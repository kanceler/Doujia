...
func NewBootstrapWithOptions(projectsRoot string, options BootstrapOptions) (*Bootstrap, error) {
	...
	return newBootstrap(projectsRoot, runRepo, instanceRepo, taskRepo, artifactRepo, eventRepo, doujiaGitRepo, options)
}
...
func NewSQLiteBootstrapWithOptions(ctx context.Context, projectsRoot string, dbPath string, options BootstrapOptions) (*Bootstrap, *sql.DB, error) {
	...
	bootstrap, err := newBootstrap(projectsRoot, repos.Runs, repos.Instances, repos.Tasks, repos.Artifacts, repos.Events, doujiaGitRepo, options)
	if err != nil {
		_ = db.Close()
		return nil, nil, err
	}
...
func newBootstrap(
	...
	orch.SetPipelineDefinitionRegistry(pipelineDefinitions)
