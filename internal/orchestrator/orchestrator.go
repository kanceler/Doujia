...
func (s *Service) OnFeedback(ctx context.Context, feedback core.TaskMetaData) error {
	...
	if !ok {
		...
		s.recordEvent(ctx, run.ID, task.ID, task.AgentID, "run_completed", "run completed", map[string]any{
			"task_id": task.ID,
		})
		return s.runs.Update(ctx, run)
	}
...
func (s *Service) handleDynamicTaskFeedback(ctx context.Context, run core.PipelineRun, task core.Task, feedback core.TaskMetaData) error {
	...
	if isGlobalTestTask(task, feedback) {
		...
		s.recordEvent(ctx, run.ID, task.ID, task.AgentID, "run_completed", "run completed after global test_code", map[string]any{
			"task_id": task.ID,
		})
		return s.runs.Update(ctx, run)
	}
...
func (s *Service) completeRunForRootInstance(ctx context.Context, run core.PipelineRun, instance core.PipelineInstance) error {
	...
	latest.Status = core.RunStatusCompleted
	latest.UpdatedAt = time.Now().UTC()
	if s.logger != nil {
		_ = s.logger.Log(run.ID, "Orchestrator", "run completed")
	}
	s.recordEvent(ctx, run.ID, "", "", "run_completed", "run completed", map[string]any{
		"pipeline_instance_id": instance.ID,
		"pipeline_id":          instance.PipelineID,
	})
	return s.runs.Update(ctx, latest)
}
...
func aggregateStateReady(state pipeline.StateSpec, instance *core.PipelineInstance) bool {
	...
	case "join_bags":
		...
		for _, exposed := range state.Exposes.Bags {
			...
			syntheticID := syntheticAggregateBagID(instance.ID, state.ID, exposed.Name)
...
func (s *Service) materializeAggregateBags(ctx context.Context, run core.PipelineRun, instance core.PipelineInstance, state pipeline.StateSpec) error {
	...
	for _, exposed := range state.Exposes.Bags {
		...
		bagID := syntheticAggregateBagID(instance.ID, state.ID, exposed.Name)
...
func syntheticAggregateBagID(instanceID core.PipelineInstanceID, stateID string, name string) string {
	return fmt.Sprintf("aggregate:%s:%s:%s", instanceID, sanitizeIDPart(stateID), sanitizeIDPart(name))
}
