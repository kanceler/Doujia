...
func TestResumeFromRefMaterializesCompletedRun(t *testing.T) {
	...
	if result.MaterializedTasks != 1 || result.MaterializedInstances != 1 || result.RunStatus != core.RunStatusCompleted {
		t.Fatalf("ResumeFromRef() = %#v, want one task and one instance materialized into completed run", result)
	}
	run, err := runRepo.Get(ctx, runID)
	if err != nil {
		t.Fatalf("Get(run) error = %v", err)
